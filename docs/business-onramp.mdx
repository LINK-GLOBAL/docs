...
Initiate a fiat → stablecoin conversion via LINK's OTC desk. You provide the core transaction details plus your own banking details (the account you are paying **from**). LINK notifies liquidity partners via Telegram; once a partner accepts, their settlement account details are delivered to you through a `banking_details_received` webhook.
...
| `currency`       | string | ✓        | Fiat currency being sent (e.g., `NGN`, `USD`, `GBP`). See [Supported Currencies](/docs/supported-currency-methods). |
...
1. **Partner notification** — Liquidity partners are alerted on Telegram with your transaction details.
2. **Banking details received** — A partner accepts and sends their settlement account. LINK fires a `banking_details_received` webhook to your registered URL:
...
3. **Send funds** — Transfer fiat to the partner's account provided in the webhook.
4. **Completion** — Once the partner confirms receipt, LINK fires a `transaction_status_updated` webhook with `partner_status: "completed"`.
